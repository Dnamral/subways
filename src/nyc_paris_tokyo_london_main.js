...
const londonGroup = outerGroup.append("g").attr("id", "london");
...
// helper near the top:
const londonStationName = p =>
  (p && (p["name:en"] || p.name || p.ref || p.operator)) || "(unnamed)";
...
const londonProjection = d3.geoMercator()
  .center([-0.12, 51.507])   // central London
  .scale(120000)
  .translate([width * 0.5, height * 0.25]);

const tokyoPath = d3.geoPath().projection(tokyoProjection);
...
Promise.all([
  ...
  d3.json("./data/london/london_tube_routes.geojson"),
  d3.json("./data/london/london_tube_stations.geojson"),
]).then(([parisRoutes, parisStations, nycRoutes, nycStations, tokyoRoutes, tokyoStations, londonRoutes, londonStations]) => {
  ...
  londonGroup.selectAll(".london-route")
    .data(londonRoutes.features)
    .enter().append("path")
    .attr("class","london-route")
    .attr("d", londonPath)
    .attr("stroke","#0057b8")
    .attr("fill","none")
    .attr("stroke-width",1.2);
  
  // stations with tooltip
  londonGroup.selectAll(".london-station")
    .data(londonStations.features)
    .enter().append("circle")
    .attr("class","london-station")
    .attr("r", 2)
    .attr("fill", "#0057b8")
    .attr("cx", d => londonProjection(d.geometry.coordinates)[0])
    .attr("cy", d => londonProjection(d.geometry.coordinates)[1])
    .on("mousemove", (event, d) => {
      const name = londonStationName(d.properties);
      tooltip.style("display","block")
        .style("left", (event.pageX + 8) + "px")
        .style("top",  (event.pageY) + "px")
